...
## 2.5. Проектирование физической модели базы данных
...
Листинг 1 - создание таблицы users (отображение логической сущности Пользователи)
...
В приведенном выше листинге можно видеть, что запрос устанавливает однозначное соответствие между атрибутами сущности Пользователи и создаваемыми полями таблицы. Например, атрибут "Отображение сообщений" (определяет возможность связаться с пользователем с помощью сообщений) здесь реализован следующим образом:

```sql
ud_messages VARCHAR(1) NOT NULL
```

Тип данных `VARCHAR(1)` выбран потому, что позволяет хранить достаточное количество вариантов настройки параметра "Отображение сообщений" и требует небольшое количество выделенной памяти под одну запись.
