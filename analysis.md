...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id | Уникальный идентификатор пользователя | Число(8) |
| Фамилия | Часть имени пользователя | Текст(32) |
| Имя | Часть имени пользователя | Текст(32) |
| Отчество | Часть имени пользователя | Текст(32) |
| Аватар | Адрес файла с картинкой-аватаром в файловой системе | Текст(12) |
| Пол | Пол пользователя, 0 - мужской, 1 - женский | Число(1) |
| Дата рождения | Дата рождения пользователя | Дата |
| Страна | Страна проживания | Текст(32) |
| Город | Город проживания | Текст(32) |
| Телефон | Личный номер телефона | Число(10) |
| О себе | Произвольно заполняемая пользователем личная информация | Текст(256) |
| Пароль | Ключевое слово для ограничения доступа к странице | Текст(256) |
| Отображение пола | Отображать ли пол на странице, 0 - видит только владелец страницы, 1 - видят друзья, 2 - видят все | Число(1) |
| Отображение ДР | Отображать ли дату рождения на странице  | Число(1) |
| Отображение аватара | Отображать ли аватар на странице | Число(1) |
| Отображение друзей | Отображать ли список друзей на странице  | Число(1) |
| Отображение групп | Отображать ли список групп на странице | Число(1) |
| Отображение записей | Отображать ли список записей на странице | Число(1) |
| Отображение сообщений | Возможность связаться с пользователем с помощью сообщений | Число(1) |
| Уведомления | Показывать ли пользователю уведомления о входящих сообщениях, 0 - нет, 1 - да | Число(1) |
| Звуки | Воспроизводить ли звук уведомления | Число(1) |
| Содержимое | Отображать ли текст уведомления | Число(1) |
| Друзья | Список id друзей | Число(8) |
| Исходящие заявки | Исходящие заявки на добавление в друзья (id пользователей) | Число(8) |
| Входящие заявки | Входящие заявки на добавление в друзья (id пользователей) | Число(8) |
| Подписки | Список id групп, на которые подписан пользователь | Число(8) |
| Записи | Текст записей на странице пользователя | Текст(256) |
| Документы | Документы, прикрепленные к определенной записи на странице | Текст(12) |
| Оцененные записи | Записи пользователей и групп, которые оценил пользователь | Число(1) |
...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id | Уникальный идентификатор группы | Число(8) |
| Название | Имя группы | Текст(32) |
| Аватар | Адрес файла с картинкой-аватаром в файловой системе | Текст(12) |
| id владельца | Уникальный идентификатор пользователя | Число(8) |
| О группе | Произвольно заполняемая пользователем информация о тематике группы | Текст(256) |
| Записи | Текст записей на странице группы | Текст(256) |
| Документы | Документы, прикрепленные к определенной записи на странице | Текст(12) |
...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id сообщения | Уникальный идентификатор сообщения | Число(8) |
| id отправителя | id пользователя, отправившего сообщение | Число(8) |
| id получателя | id пользователя, которому предназначено сообщение | Число(8) |
| Текст сообщения | Содержимое сообщения | Текст(256) |
| Документы | Прикрепленные к сообщению документы (адрес) | Текст(12) |
...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id пользователя | id пользователя | Число(8) |
| id друга | Один друг из списка друзей | Число(8) |
...
| - | - | - |
| id пользователя | id пользователя | Число(8) |
| id получателя | Исходящая заявка на добавление в друзья (id пользователя, которому отправлена заявка данным пользователем) | Число(8) |
...
| - | - | - |
| id пользователя | id пользователя | Число(8) |
| id получателя | Входящая заявка на добавление в друзья (id пользователя, который отправил заявку данному пользователю) | Число(8) |
...
| - | - | - |
| id пользователя | id пользователя | Число(8) |
| id группы | id группы, на которую подписан пользователь | Число(8) |
...
| - | - | - |
| id пользователя | id пользователя | Число(8) |
| id записи | Выложенная на странице пользователя запись | Текст(256) |

#### 2.2.6.Документы пользователя

| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id записи | id выложенной на странице пользователя записи | Число(8) |
| Документ | Документ, прикрепленные к записи на странице пользователя | Текст(12) |
...
| - | - | - |
| id пользователя | id пользователя | Число(8) |
| id записи | Выложенная на странице пользователя или группы запись | Число(8) |
...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id | id пользователя или группы | Число(8) |
| id записи | Выложенная на странице пользователя или группы запись | Текст(256) |
...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id записи | id выложенной на странице пользователя или группы записи | Число(8) |
| Документ | Документ, прикрепленные к записи на стене пользователя или группы | Текст(12) |
...
| Имя атрибута | Краткое описание | Домен |
| - | - | - |
| id сообщения | id отправленного сообщения | Число(8) |
| Документ | Документ, прикрепленные к сообщению | Текст(12) |
...
| Имя атрибута | Краткое описание | Домен | Первичный ключ |
| - | - | - | - |
| id | Уникальный идентификатор пользователя | Число(8) | + |
| Фамилия | Часть имени пользователя | Текст(32) |  |
| Имя | Часть имени пользователя | Текст(32) |  |
