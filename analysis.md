...
Для хранения информации об изображении пользователя (аватар) используется отношение "Документы". Таким образом, этот атрибут хранит идентификаторы документов из отношения "Документы".

| id   | Фамилия | Имя  | Аватар | Пол | Дата рождения | Страна | Город | Телефон | О себе |
| -    | -       | -    | -      | -   | -             | -      | -     | -       | -      |
| 0001 | Иванов  | Иван | 0001   | m   | 10.10.2000    | Россия | Муром | 1488228 | text   |
| 0002 | Петров  | Петр | null   | m   | 10.10.1990    | Россия | Выкса | 1337882 | text   |

#### 2.1.2.Пароли

Отношение хранит пароли, которые пользователи используют для входа в систему. В базе данных пароли хранятся в зашифрованном виде; шифрование и дешифрование происходят на стороне клиента.
...
Сокращение О. от слова "отображение".
...
Конфигурация клиента хранится в базе данных, а не на клиенте, поскольку при изменении (повреждении конфигурации, переустановке, установке на новое устройство) программы-клиента ее поведение должно оставаться прежним.
...
Для хранения информации об изображении группы (аватар) используется отношение "Документы". Таким образом, этот атрибут хранит идентификаторы документов из отношения "Документы".

| id   | Название | Аватар | id пользователя | О группе |
| -    | -        | -      | -               | -        |
| 0003 | gr01     | 0002   | 0001            | text     |
| 0004 | gr02     | null   | 0002            | text     |

#### 2.1.6.Список друзей

Отношение устанавливает соответствие id (пользователя) - id (пользователя), позволяющее указать наличие связи "дружба" между двумя пользователями.

Для каждой такой связи создается две записи, по одной на каждого пользователя. Следовательно, при таком хранении связи "дружба" присутствует дублирование данных. Это дублирование неизбежно, поскольку при добавлении новой пары пользователь-пользователь в данное отношение возникает неоднозначность: "id" какого из них должно храниться в атрибуте id, а какого в атрибуте "id друга".
...
Хранит содержимое записи, опубликованной на странице пользователя или группы.
...
| id сообщения | id исходящее | id назначения | Текст |
| -            | -            | -             | -     |
| 0001         | 0001         | 0002          | text  |
| 0002         | 0001         | 0002          | text  |
| 0003         | 0005         | 0001          | text  |
...
#### 2.1.15.Документы и сообщения

Устанавливает соответствие между сообщением и прикрепленными к нему документами.

| id сообщения | id документа |
| -            | -            |
| 0001         | 0001         |
| 0002         | 0001         |

#### 2.1.16.Идентификаторы

Отношение устанавливает соответствие между идентификатором (id) и его принадлежностью одному из типов. Для упрощения базы данных можно было использовать два разных класса идентификаторов, но это усложняет формирование адреса страницы (например, тогда адрес группы будет выглядеть как gr0000, а личной страницы - us0000; в выбранном случае адреса находятся в одном множестве уникальных элементов, тогда они выглядят как 0000 и 0001 соответственно). В качестве атрибута, хранящего информацию о типе id, используется строковый тип данных, а не булевый, поскольку в этом случае отношение в частности и база данных в целом смогут хранить индентификаторы не только для личных страниц и групп. Это может пригодиться при расширении функциональности информационной системы.
